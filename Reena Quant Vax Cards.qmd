...
```{r}
library(readr)
QUANT_vax_cards <- read_csv("RStudio_GitHub Merged Data (QUANT) - A+B Vax Cards.csv", 
...
```{r}
library(readr)
Consolidated_QUANT_Vax_Cards <- read_csv("Consolidated QUANT Vax Cards (no refuerzo, flu, vitA) - A+B Vax Cards MINUS Refuerzos, HepBs, Varicella.csv", 
    col_types = cols(date = col_date(format = "%d-%m-%y"), 
